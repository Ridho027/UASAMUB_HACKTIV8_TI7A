package com.asus.uas_amub_hacktiv8_ti7a;
...
public class MyPelatihanDetailAct extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        reference = FirebaseDatabase.getInstance().getReference().child("Program").child("Javascript");
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                xnama_pelatihan.setText(dataSnapshot.child("nama_program").getValue().toString());
                xlokasi.setText(dataSnapshot.child("lokasi").getValue().toString());
                xtime_pelatihan.setText(dataSnapshot.child("time_program").getValue().toString());
                xdate_pelatihan.setText(dataSnapshot.child("date_program").getValue().toString());
